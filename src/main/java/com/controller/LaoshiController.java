...
public class LaoshiController {
    ...
    public R login(String username, String password, String role, HttpServletRequest request) {
        LaoshiEntity user = laoshiService.selectOne(new EntityWrapper<LaoshiEntity>().eq("account", username));
        if(user != null){
    ...
    public R register(@RequestBody LaoshiEntity user){
        if(laoshiService.selectOne(new EntityWrapper<LaoshiEntity>().eq("account", user.getAccount())) !=null) {
            return R.error("老师已存在");
        }
        user.setRole("老师");
        laoshiService.insert(user);
        return R.ok();
    }
    ...
    public R resetPass(String username, HttpServletRequest request){
        LaoshiEntity user = laoshiService.selectOne(new EntityWrapper<LaoshiEntity>().eq("username", username));
        if(user==null) {
            return R.error("账号不存在");
        }
        user.setPassword("123456");
        laoshiService.update(user,null);
    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        if(role.equals("老师")){
            ...
            page = laoshiService.queryPage(params);
        }else{
            page = laoshiService.queryPage(params);
        }
